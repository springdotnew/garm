...
type recordingProviderHelper struct {
	testProviderHelper

	mux       sync.Mutex
	updates   []statusUpdate
	updateErr func(statusUpdate) error
}
...
func (h *recordingProviderHelper) SetInstanceStatus(_ string, status commonParams.InstanceStatus, _ []byte, force bool) error {
	update := statusUpdate{status: status, force: force}
	h.mux.Lock()
	h.updates = append(h.updates, update)
	h.mux.Unlock()
	if h.updateErr != nil {
		return h.updateErr(update)
	}
	return nil
}

func (h *recordingProviderHelper) recordedUpdates() []statusUpdate {
	h.mux.Lock()
	defer h.mux.Unlock()
	return append([]statusUpdate(nil), h.updates...)
}
...
func TestNormalDeleteFailurePreservesConcurrentForceDelete(t *testing.T) {
	deleteStarted := make(chan struct{})
	releaseDelete := make(chan struct{})
	...
	helper := &recordingProviderHelper{
		updateErr: func(update statusUpdate) error {
			if update.status == commonParams.InstancePendingDelete {
				return runnerErrors.NewInstanceTransitionError(
	...
	<-deleteStarted
	require.NoError(t, manager.handleUpdate(dbCommon.ChangePayload{
		EntityType: dbCommon.InstanceEntityType,
		Operation:  dbCommon.UpdateOperation,
		Payload: params.Instance{
			Name:       "runner-name",
	...
	require.Equal(t, []statusUpdate{
		...
		{status: commonParams.InstancePendingDelete, force: false},
	}, helper.recordedUpdates())

	manager.mux.Lock()
	status := manager.instance.Status
	manager.mux.Unlock()
	require.Equal(t, commonParams.InstancePendingForceDelete, status)
}
