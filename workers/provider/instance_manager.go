...
func (i *instanceManager) consolidateState() error {
	...
	case commonParams.InstancePendingDelete, commonParams.InstancePendingForceDelete:
		...
		if err := i.handleDeleteInstanceInProvider(instance); err != nil {
			slog.ErrorContext(i.ctx, "deleting instance in provider", "error", err, "forced", instance.Status == commonParams.InstancePendingForceDelete)
			if prevStatus == commonParams.InstancePendingDelete {
				i.incrementBackOff()
				if statusErr := i.helper.SetInstanceStatus(instance.Name, commonParams.InstancePendingDelete, []byte(err.Error()), false); statusErr != nil {
					var transitionErr *runnerErrors.InstanceTransitionError
					if errors.As(statusErr, &transitionErr) &&
						transitionErr.From == commonParams.InstancePendingForceDelete &&
						transitionErr.To == commonParams.InstancePendingDelete {
						return fmt.Errorf("error removing instance after force deletion was requested: %w", err)
					}
					return fmt.Errorf("setting instance status to error: %w", statusErr)
				}
